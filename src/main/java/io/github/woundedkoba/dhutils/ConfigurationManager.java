...
public class ConfigurationManager {
    ...
    private void setItem(String key, String val) {
        ...
        Debugger.getInstance().debug(2,
                "setItem: key = " + key + ", val = " + val + ", class = " + c.getName());
        ...
        if (val == null) {
            ...
            // String config values are common, so this should be a little
            // quicker than going
            // through the default case below (using reflection)
            processedVal = val;
        } else if (Enum.class.isAssignableFrom(c)) {
            // this really isn't very pretty, but as far as I can tell there's
            // no way to
            ...
            try {
                ...
                throw new DHUtilsException("'" + val + "' is not a valid value for '"
                        + key + "'");
            }
        } else {
            // the class we're converting to must either have a static method
            // annotated with @FactoryMethod,
        ...
        if (processedVal != null || val == null) {
            if (listener != null && validate) {
                processedVal = listener.onConfigurationValidate(this, key, get(key),
                        processedVal);
            }
    ...
    private <T> void setItem(String key, List<T> list) {
        ...
        if (!(config.getDefaults().get(keyPrefixed) instanceof List<?>)) {
            throw new DHUtilsException("Key '" + key
                    + "' does not accept a list of values");
        }
        if (listener != null && validate) {
